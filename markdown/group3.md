...
To get practice doing this, take the following proof that $\neg P(n) \rightarrow \neg P(n-1)$ (that might be part of a proof by contradiction using well-ordering) and transform it into a proof that $P(n)\rightarrow P(n+1)$ (that might be the inductive case in a proof by induction) (**Problem 3**). In this case, we define $P(n)$ to be $\Sigma_{i=0}^{n} i = \frac{n(n+1)}{2}$.
