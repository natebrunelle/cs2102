...
To prove that this code always terminates using induction, we first need to identify a predicate $P$ so that we can state a theorem of form $\forall n\in \mathbb{N}. P(n)$. In this case we'll define $P(n)$: "after the nth iteration of the for loop, $m$ is equal to the maximum of the first $n+1$ elements of the list". To begin, show a base case that $P(0)$ is true (this is **Problem 6**).
