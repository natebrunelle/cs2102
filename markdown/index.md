...
Course meetings are synchronous and may be attended in-person or, on occasion, remotely via zoom live-stream due to illness or some other extenuating circumstance. Note that in-class attendance is strongly encouraged, and a "grade-round-up" incentive may be awarded for participation. We will additionally post recordings of live-streams after class for asynchronous review.
...
From time to time we will refer to the following writeups intended to supplement the textbooks ([MCS](files/mcs.pdf) and [∀x](files/forallx.pdf), the latter also having a [solution book](http://forallx.openlogicproject.org/solutions/forallxsol.pdf));
Here they are:
